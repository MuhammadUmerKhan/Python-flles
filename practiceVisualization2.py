...
def graph_data(input_region, entered_year):
    region_data = df[df['Region'] == input_region]
    year_data = region_data[region_data['Year']==entered_year]
    ...
    fig1 = px.pie(est_data, names='Month', values='Estimated_fire_area',
                  title="{} : Monthly Average Estimated Fire Area in year {}".format(input_region, entered_year))
    fig2 = px.bar(veg_data, x='Month', y='Count',
                  title='{} : Average Count of Pixels for Presumed Vegetation Fires in year {}'.format(input_region, entered_year))
    return [
