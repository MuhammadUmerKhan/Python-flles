...
app.layout = html.Div(children=[ html.H1("Flight Details Statistics Dashboard", 
                                ...
                                html.Div([
                                  html.Div(dcc.Graph(id='nas-plot')),
                                  html.Div(dcc.Graph(id='seacurity-plot')),
                                ], style={'display':'flex'}),
                                html.Div(dcc.Graph(id='late-plot'),
                                          style={'width':'100%'})
            
])
def data(airline_data, entered_year):
  df =  airline_data[airline_data['Year']==int(entered_year)]
  avg_car = df.groupby(['Month','Reporting_Airline'])['CarrierDelay'].mean().reset_index()
  avg_weather = df.groupby(['Month','Reporting_Airline'])['WeatherDelay'].mean().reset_index()
  avg_NAS = df.groupby(['Month','Reporting_Airline'])['NASDelay'].mean().reset_index()
...
@app.callback([
  ...
  Output(component_id='weather-plot', component_property='figure'),
  Output(component_id='nas-plot', component_property='figure'),
  Output(component_id='seacurity-plot', component_property='figure'),
  Output(component_id='late-plot', component_property='figure'),
],
  Input(component_id='input-year', component_property='value')
)
def figures(entered_year):
  avg_car, avg_weather, avg_NAS, avg_sec, avg_late = data(airline_data, entered_year)
  ...
  sec_fig = px.bar(avg_sec, x='Month', y='SecurityDelay', color='Reporting_Airline', title='Average security delay time (minutes) by airline')
